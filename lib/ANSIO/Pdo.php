...
class Pdo
{
    ...
    public function addEntity($entity, $fields, $onDuplicate = null)
    {
    ...
    public function add($fields = array(), $onDuplicate = null)
    {
        ...
        $statement = $this->pdo->prepare($query);
        $exec = $statement->execute($fields);
        $lastInsertId = $this->pdo->lastInsertId();
        return $lastInsertId ? $lastInsertId : $exec;
    }
    ...
    public function fetchBySql($query)
    {
        $statement = $this->pdo->prepare($query);
        $statement->execute();
        $statement->setFetchMode(\PDO::FETCH_ASSOC);
    ...
    public function execBySql($query)
    {
        $statement = $this->pdo->prepare($query);
        return $statement->execute();
    }
    ...
    public function fetchFields($query)
    {
        ...
        for ($i = 0; $i < $statement->columnCount(); $i++) {
            $fields[$statement->getColumnMeta($i)['name']] = $statement->getColumnMeta($i);
